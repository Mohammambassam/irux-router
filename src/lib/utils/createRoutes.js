...
import { validateDynamicPath } from "./validateDynamicPath";
...
export function createRoutes(routes) {
  ...
  const notFoundIndex = normalizedRoutes.findIndex(
    (route) => route.path === "*"
  );

  if (
    notFoundIndex !== -1 &&
    notFoundIndex !== normalizedRoutes.length - 1
  ) {
    routerError(
